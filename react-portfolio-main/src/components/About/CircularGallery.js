...
class Media {
   ...
   createDesc() { 
      // Asegúrate de que this.mesh esté definido
      if (!this.mesh) {
         return;
      }

      // Crear una textura para la descripción
      const { texture, width, height } = createTextTexture(
         this.gl,
         this.desc, // Usar la descripción
         "20px Arial", // Tamaño y fuente de la descripción
         this.textColor
      );
      ...
      this.descMesh = new Mesh(this.gl, { geometry, program });
      const aspect = width / height;
      const descHeight = this.plane.scale.y * 0.1; // Altura de la descripción
      const descWidth = descHeight * aspect;

      this.descMesh.scale.set(descWidth, descHeight, 1);
      this.descMesh.position.y = this.mesh.position.y - descHeight - 0.1; // Posición debajo del título
      this.descMesh.setParent(this.plane);
   }
